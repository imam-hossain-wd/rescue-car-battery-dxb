...
export default function EmergencyTrustBanner() {
  ...
  const majorRoads = ["SZR", "MBZ Road", "Al Khail Road"];

  return (
    <section className="w-full bg-[#090B0D] py-8 relative overflow-hidden border-y border-white/5">
      {/* Background decorative */}
      <div className="absolute inset-0 bg-grid-white/[0.02]"></div>
      <div className="absolute top-0 right-0 w-72 h-72 bg-[#FFC400]/8 rounded-full blur-3xl"></div>
      <div className="absolute bottom-0 left-0 w-72 h-72 bg-[#FFC400]/5 rounded-full blur-3xl"></div>

      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 items-center max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-5">
            ...
            <div className="inline-flex items-center gap-2 bg-red-500/10 border border-red-500/20 rounded-full px-3 py-1 mb-3">
              ...
              <span className="text-[10px] font-semibold text-red-400 uppercase tracking-wider">🚨 Emergency Response</span>
            </div>

            {/* Heading */}
            <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-white leading-tight mb-3">
              Stuck on <span className="text-[#FFC400]">SZR</span> or Your Driveway? We're On Standby.
            </h2>

            {/* Body */}
            <p className="text-gray-400 text-xs sm:text-sm leading-relaxed mb-4">
              UAE summer heat is the #1 cause of battery failure. No towing fees. No hours-long waits. Our mobile vans are stationed across Dubai for on-the-spot jumpstarts & replacements in under 15 minutes.
            </p>

            {/* Major Roads */}
            <div className="flex flex-wrap items-center gap-1.5 mb-5">
              <span className="text-[10px] text-gray-500 font-medium uppercase tracking-wider">Roads:</span>
              {majorRoads.map((road, index) => (
                <span
                  key={index}
                  className="inline-flex items-center gap-1 bg-white/5 border border-white/10 rounded-full px-2.5 py-0.5 text-[10px] font-medium text-gray-300"
                >
                  <RiMapPin2Line className="w-2.5 h-2.5 text-[#FFC400]" />
                  {road}
                </span>
              ))}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <Button asChild className="bg-[#FFC400] hover:bg-[#FFC400]/90 text-[#090B0D] font-bold px-5 py-2.5 rounded-full shadow-lg shadow-[#FFC400]/20 hover:shadow-[#FFC400]/40 transition-all duration-200 group text-sm">
                <Link href="#" className="flex items-center gap-2">
                  <RiFlashlightLine className="w-4 h-4" />
                  Get Emergency Help
              ...
              <Link
                href="#"
                className="inline-flex items-center gap-1.5 text-xs font-medium text-gray-400 hover:text-[#FFC400] transition-colors duration-200"
              >
          ...
          <div className="lg:col-span-7">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {trustMetrics.map((metric, index) => (
                <div
                  key={index}
                  className={`group relative bg-[#0D1117] rounded-xl p-4 text-center transition-all duration-300 border ${metric.borderColor} hover:border-[#FFC400]/50 hover:-translate-y-1`}
                >
                  <div className="w-9 h-9 mx-auto mb-2 rounded-lg bg-white/5 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                    <metric.icon className={`w-4.5 h-4.5 ${metric.iconColor}`} />
                  </div>
                  <div className="text-base sm:text-lg font-bold text-white group-hover:text-[#FFC400] transition-colors duration-300">
                    {metric.value}
                  </div>
            ...
            <div className="flex items-center justify-center gap-2 mt-4">
              ...
              <span className="text-[10px] text-gray-500">
                24/7 Active • Average arrival 5–15 minutes*
              </span>
            </div>
          </div>
        </div>
