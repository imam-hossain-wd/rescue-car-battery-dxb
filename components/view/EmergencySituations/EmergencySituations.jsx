"use client";

import { 
  RiHomeLine, 
  RiBuildingLine, 
  RiRoadMapLine, 
  RiShoppingBag3Line,
  RiParkingLine,
  RiPlaneLine,
  RiHotelLine,
  RiCommunityLine,
  RiMoonLine,
  RiArrowRightLine,
  RiMapPin2Line
} from "@remixicon/react";
import { Button } from "@/components/ui/button";
import Link from "next/link";

export default function EmergencySituations() {
  const situations = [
    {
      icon: RiHomeLine,
      title: "Stuck at Home",
      description: "Dead battery in your driveway or garage?",
      color: "from-blue-500/20 to-blue-500/5",
      iconColor: "text-blue-400",
      emoji: "🏠",
    },
    {
      icon: RiBuildingLine,
      title: "Office Parking",
      description: "Battery died during work hours?",
      color: "from-indigo-500/20 to-indigo-500/5",
      iconColor: "text-indigo-400",
      emoji: "🏢",
    },
    {
      icon: RiRoadMapLine,
      title: "Roadside Breakdown",
      description: "Stranded on the side of the road?",
      color: "from-red-500/20 to-red-500/5",
      iconColor: "text-red-400",
      emoji: "🛣️",
    },
    {
      icon: RiShoppingBag3Line,
      title: "Shopping Mall Parking",
      description: "Battery died while shopping?",
      color: "from-pink-500/20 to-pink-500/5",
      iconColor: "text-pink-400",
      emoji: "🛍️",
    },
    {
      icon: RiParkingLine,
      title: "Underground Basement",
      description: "Stuck in basement parking? We come to you.",
      color: "from-gray-500/20 to-gray-500/5",
      iconColor: "text-gray-400",
      emoji: "🅿️",
    },
    {
      icon: RiPlaneLine,
      title: "Airport Area",
      description: "Battery dead at the airport?",
      color: "from-cyan-500/20 to-cyan-500/5",
      iconColor: "text-cyan-400",
      emoji: "✈️",
    },
    {
      icon: RiHotelLine,
      title: "Hotel",
      description: "Staying at a hotel? We'll come to you.",
      color: "from-amber-500/20 to-amber-500/5",
      iconColor: "text-amber-400",
      emoji: "🏨",
    },
    {
      icon: RiCommunityLine,
      title: "Residential Community",
      description: "In a gated community or compound?",
      color: "from-green-500/20 to-green-500/5",
      iconColor: "text-green-400",
      emoji: "🏡",
    },
    {
      icon: RiMoonLine,
      title: "Late-Night Emergency",
      description: "24/7 support for midnight breakdowns.",
      color: "from-purple-500/20 to-purple-500/5",
      iconColor: "text-purple-400",
      emoji: "🌙",
    },
  ];

  return (
    <section className="w-full bg-linear-to-b from-white dark:from-[#090B0D] to-gray-50 dark:to-[#0D1117] py-8 relative overflow-hidden">
      {/* Background decorative elements */}
      <div className="absolute inset-0 bg-grid-black/[0.02] dark:bg-grid-white/[0.02]"></div>
      <div className="absolute top-0 left-0 w-96 h-96 bg-[#FFC400]/5 rounded-full blur-3xl"></div>
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-[#FFC400]/5 rounded-full blur-3xl"></div>
      
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative">
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
          <div className="inline-flex items-center gap-2 bg-[#FFC400]/10 border border-[#FFC400]/20 rounded-full px-4 py-1.5 mb-4">
            <RiMapPin2Line className="w-4 h-4 text-[#FFC400]" />
            <span className="text-xs font-semibold text-[#FFC400] uppercase tracking-wider">Wherever You Are</span>
          </div>
          
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#090B0D] dark:text-white mb-3">
            Wherever Your Battery Dies, <span className="text-[#FFC400]">We'll Come to You</span>
          </h2>
          
          <p className="text-gray-600 dark:text-gray-300 text-sm sm:text-base max-w-2xl mx-auto">
            No towing. No pushing the car. No searching for an open garage. Send us your location and we'll bring the service to you.
          </p>
        </div>

        {/* Situations Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 max-w-6xl mx-auto">
          {situations.map((situation, index) => (
            <div
              key={index}
              className="group relative bg-white dark:bg-[#0D1117] rounded-2xl p-5 sm:p-6 transition-all duration-300 border border-gray-200 dark:border-white/10 hover:border-[#FFC400]/50 hover:shadow-xl hover:shadow-[#FFC400]/5 hover:-translate-y-1 cursor-pointer"
            >
              {/* Gradient background on hover */}
              <div className={`absolute inset-0 bg-linear-to-br ${situation.color} opacity-0 group-hover:opacity-100 rounded-2xl transition-opacity duration-300`}></div>
              
              <div className="relative z-10 flex items-start gap-4">
                {/* Icon */}
                <div className="shrink-0">
                  <div className="w-12 h-12 rounded-xl bg-gray-100 dark:bg-white/5 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                    <span className="text-2xl">{situation.emoji}</span>
                  </div>
                </div>

                {/* Content */}
                <div className="flex-1 min-w-0">
                  <h3 className="text-base font-bold text-[#090B0D] dark:text-white group-hover:text-[#FFC400] transition-colors duration-300">
                    {situation.title}
                  </h3>
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-0.5">
                    {situation.description}
                  </p>
                </div>

                {/* Arrow indicator */}
                <div className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <RiArrowRightLine className="w-4 h-4 text-[#FFC400]" />
                </div>
              </div>
            </div>
          ))}
        </div>

        {/* Bottom CTA */}
        <div className="text-center mt-12 sm:mt-16">
          <div className="inline-flex flex-wrap items-center justify-center gap-4">
            <Button asChild className="bg-[#FFC400] hover:bg-[#FFC400]/90 text-[#090B0D] font-bold px-8 py-3 rounded-full shadow-lg shadow-[#FFC400]/20 hover:shadow-[#FFC400]/40 transition-all duration-200 group">
              <Link href="#" className="flex items-center gap-2">
                Send Your Location
                <RiMapPin2Line className="w-4 h-4 group-hover:translate-x-0.5 transition-transform duration-200" />
              </Link>
            </Button>
            <Button asChild variant="outline" className="border-2 border-[#090B0D] dark:border-white hover:bg-[#090B0D] hover:text-white dark:hover:bg-white dark:hover:text-[#090B0D] px-8 py-3 rounded-full font-semibold transition-all duration-200">
              <Link href="#" className="flex items-center gap-2">
                Call Emergency
                <RiArrowRightLine className="w-4 h-4" />
              </Link>
            </Button>
          </div>
          
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-4 flex items-center justify-center gap-1">
            <span className="inline-block w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span>
            Emergency response available 24/7 across all Dubai locations
          </p>
        </div>
      </div>
    </section>
  );
}